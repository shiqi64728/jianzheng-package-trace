...
| GOAL-04 | RISK-02 | D02/D03 failure audit | MUST | PASS | val 失败分类、D02 ≥50 GT、跨 split 近重复 | 审计完成且 test 预测不用于调参 | 1,299 条 val failure；50 个 D02 GT；711 图跨 split DCT audit，0 exact、3 perceptual near duplicates | `evidence/detector-error-audit-v1.1.*`；`evidence/near-duplicate-audit-v1.1.json` | 无；3 组疑似 leakage 只报告不删除 | 无 |
